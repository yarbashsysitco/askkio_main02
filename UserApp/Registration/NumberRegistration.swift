...
struct NumberRegistration: View {
    @StateObject private var viewmodel = NumberRegistrationViewModel()
    var body: some View {
        NavigationView {
            VStack {
                VStack {
                    ...
                    CountryCode(mobileText: $viewmodel.mobileText, isInputValid: $viewmodel.isInputValid)
                    Spacer().frame(height: 5)
                    Underline(isInputValid: $viewmodel.isInputValid)
                    Spacer().frame(height: 5)
                    ChooseOptionLbl()
                    Spacer().frame(height: 20)
                    TermsAbdConditionsLbl()
                    LoderView(loadingView: $viewmodel.showLoader) // Using showLoader to toggle LoaderView
                    Spacer()
                }
                HStack {
                    Spacer()
                    NavigateButtonClick(mobileText: $viewmodel.mobileText,
                                        isInputValid: $viewmodel.isInputValid,
                                        showLoader: $viewmodel.showLoader,
                                        isNumberRegistrationPresented: $viewmodel.isNumberRegistrationPresented, viewModel: viewmodel)
                }
...
struct NavigateButtonClick: View {
    @Binding var mobileText: String
        ...
        var viewModel: NumberRegistrationViewModel
